...
function buildForm(formID, shortCode) {

	if(defaultCssLoaded === false) {
        defaultCssLoaded = true;
		$("body").append('<link rel="stylesheet" href="'+appurlformbuilder+'css/common.css?v=0.1" type="text/css">');
	}
	    	...
	    	if(response.result == "true") {	
                ...
                if(formSettings.settings.form_type != "Product" && formSettings.settings.form_type != "Cart") {
	                if(formSettings.settings.popup_status == "1" || formSettings.settings.form_type == "ExitIntent") {
	                    ...
	                    html += ' <button type="button" class="close fa fa-times" data-dismiss="modal" aria-label="Close"></button>';
	                    html += '</div>';
		            ...
		            if(fieldMaxlength) {
  			        ...
  			        if(v1.field_type == "textbox") {
                        ...
                        if(fieldLabelVisibility == "visible") {
          				...
          				html += '<input type="hidden" name="'+fieldName+'" value="'+v1.field_placeholder+'">';
			        ...
			        contentType: false,
		  	        ...
		  	        success: function(response) {
		  	        	if(response.payment_method && response.payment_method == "paypal") {
			  	        	...
			  	        	if(response.result == "true") {
			  	        		obj.find('input[type=text],input[type=password],input[type=email],input[type=tel],select,textarea').val('');
